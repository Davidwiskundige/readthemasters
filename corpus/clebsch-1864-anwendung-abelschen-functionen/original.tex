...
\]
\emph{ausgedehnt über alle Schnittpunkte der Curve mit $w=0$, ein Aggregat rationaler und logarithmischer Functionen der Coefficienten von $w$.}\ednote{The numerator of the displayed integral is printed with one index throughout, $\Sigma\pm a_{i}b_{i}x_{i}\,dx_{i}$. The same sum is set $\Sigma\pm a_{1}b_{2}x_{3}\,dx_{4}$ on pp. 221 and 222, and again in the very next theorem; the alternating sum requires four distinct indices. Kept as printed.}
...
Differentiiren wir nun die Gleichungen (20.) nach den verschiedenen $x$, und setzen sodann zusammengehörige Werthsysteme ein. Aus der Differentiation der $i^{\text{ten}}$ Gleichung erhalten wir das System\ednote{The second row of the system below is printed with a roman $d$ in its last denominator, $\partial u^{(s)}/dx_{2}$. Every other denominator in the system has $\partial$, and a partial derivative is what the differentiation requires. Kept as printed.}
\[
\begin{aligned}
...
\]
Weil nun die Gleichungen bestehen:\ednote{Rows 2 and 3 below are printed with the coefficient $y_{2}$ on the second term. Euler's relation for a homogeneous function, of which row 1 is the pattern, pairs $y_{1}$ with $\partial v/\partial y_{1}$, so $y_{1}$ is what the argument requires. Kept as printed.}
\[
\begin{aligned}
...
\right.\tag{23.}
\]
wo die $\gamma$ nur noch von der \emph{Ordnung} von $w$ abhängen. Da ferner die Fläche $n^{\text{ter}}$ Ordnung\ednote{Printed $n^{\text{ter}}$. It is the cutting surface $w=0$, of order $k$, that is allowed to degenerate, and the same clause counts $k$ planes, so $k^{\text{ter}}$ is what the argument requires. Kept as printed.} als System von $k$ Ebenen erscheinen kann, so hat man offenbar
...
Unter diesen ist die Schwingungsebene\ednote{Printed Schwingungsebene, the plane of oscillation. The sentence before it, the sentence after it and the mathematics all require Schmiegungsebene, the osculating plane. Kept as printed.} im Punkte selbst nicht enthalten.
...
Werden auch die andern $u$ einander gleich, so hat man eine doppeltberührende Ebene, und für die Berührungspunkte\ednote{Equation (28.) is printed with a two-bar $=$, where (25.), (26.) and (27.) all set the three-bar $\equiv$. These are congruences with respect to the periods, not equations, so $\equiv$ is what the argument requires. Kept as printed.}
\[
