...
\origpage{21}
\begin{center}
{\LARGE\bfseries Études géométriques de quelques-unes des propriétés de deux surfaces dont les points se correspondent un-à-un.\par}
\bigskip
Par H.~G. \textsc{Zeuthen} à Copenhague.
\end{center}
...
\textbf{Propriétés des courbes qui sur l'une des deux surfaces correspondent à des sections planes de l'autre.}
\end{center}
...
\textbf{Construction et propriétés de surfaces auxiliaires.}
\end{center}
...
\textbf{Cônes circonscrits à $[F_3]$ ou à $[F_4]$; usage des surfaces auxiliaires.}
\end{center}
...
\textbf{Extensions des résultats exprimés par les équations (I), (II) et (III).}
\end{center}
...
\]
L'équation $[\mathrm{I}_a]$ nous donnera alors, si l'on y remplace $3\,n' - c'$ par la quantité $3\,a - \varkappa$, qui y est égale (équation réciproque à la $3^{\mathrm{me}}$ équation (II) de l'introduction du précédent article):
\[
a\,(n - 2) = \varkappa + \varrho + 2\,\sigma + \Sigma[x\,(\mu - 2)],
\]
qui est la $1^{\mathrm{re}}$ équation (VIII) du précédent article.
...
ou bien, comme
\[
\varkappa = c' + 3\,a - 3\,n' \quad\text{[équation réciproque à la $3^{\mathrm{me}}$ (II)]},
\]
\[
...
\]
Or on a pour chaque point conique de $[F]$ et pour chaque plan tangent à $[F]$ le long d'une courbe [$2^{\mathrm{me}}$ équation (VII)]
\[
\begin{aligned}
v &= z + 3\,x - 3\,\mu, \\
v' &= z' + 3\,x' - 3\,\mu';
...
\textbf{Addition sur la représentation d'une surface sur un plan multiple.}
\end{center}
...
\textbf{*)} Abhandlungen der Gesellschaft der Wissenschaften zu G\"ottingen 15.~Bd.
