...
- [ ] 6.4 Review checkpoint, then a DCO-signed PR for the Clebsch completion — **the Clebsch PR is #42, already open**; this pushes the completion to its branch rather than opening a new one. **Deliberately left open at archive time.** Everything is committed locally and verified (gate passes on 13 works, 193 tests pass, all 1383 Clebsch math spans render), but pushing is outward-facing and was held for the contributor's go-ahead. Two branches are waiting: `transcribe/clebsch-1864-pages-207-222` → PR #42 (corpus only), and `impl/transcribe-per-batch-subagents` → PR #41 (everything else). Archiving the change does not depend on the push, and the work it records is done
